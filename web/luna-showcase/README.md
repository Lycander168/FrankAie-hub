...
| glb Draco 壓縮 | @gltf-transform/cli | MIT |
...
原始檔為 SolidWorks STEP（AP214，含手機/手錶/耳機示意件）。已轉成 `public/models/luna.glb`
（39 個零件節點 / 46 primitives，可獨立炸開做 exploded-view 組裝動畫），並經 **Draco 壓縮**：
**2.63 MB → 575 KB（−78%）**。STEP 沒有材質，材質在 `model.js` 載入時套上品牌化深色金屬。

Draco 解碼器已**內建**於 `public/draco/`（`draco_decoder.wasm` 等），`model.js` 的 `DRACOLoader`
指向 `./draco/`，**不依賴外部 CDN** —— 整包搬進 SHOPLINE 官網也能離線運作。
...
# 2) Draco 壓縮（用 draco 而非 optimize，避免合併零件、保住 exploded view）
npm i -D @gltf-transform/cli
npx gltf-transform draco public/models/luna.glb public/models/luna.glb
npx gltf-transform inspect public/models/luna.glb   # 確認 KHR_draco_mesh_compression、零件數不變
```

> 原始 STEP 不入庫（見 `.gitignore`），由設計端保管；執行只需 glb + `public/draco/`。
...
- [x] Draco 壓縮 glb（2.63 MB → 575 KB），解碼器內建於 `public/draco/`。
