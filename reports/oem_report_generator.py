...
concl = tbl([
    ...
    ['損益兩平', '約 650 台（募資額約 NT$98 萬）'],
    ['基準情境損益', '900 台、營收 NT$135 萬，稅前淨利約 NT$13 萬（約 10%）；樂觀 2,000 台淨利約 NT$91 萬（30%）'],
    ['前期資金需求', '約 NT$40–80 萬（不含量產貨款；試產訂金另計）'],
...
story.append(tbl([
    ...
    ['<b>合計</b>', '<b>約 141,000–290,000</b>', '本模型取 NT$25 萬（含雜支）'],
], [55 * mm, 40 * mm, 75 * mm]))
story.append(Paragraph(
...
def scenario(units, avg_price, goods, order_qty, ship=70, plat_rate=0.105,
             mkt_rate=0.12, preheat=180000, onetime=250000):
    rev = units * avg_price
    plat = rev * plat_rate
...
labels = ['募資總額', '平台＋金流費', '行銷費（含預熱）', '貨物成本（依下單量）', '出貨物流', 'NRE 一次性費用', '<b>稅前損益</b>']
...
story.append(Paragraph(
    '解讀：每台邊際貢獻約 NT$650–670（售價扣平台費、貨物含 5% 備品、物流與變動行銷），'
    '對固定支出約 NT$43 萬（NRE 25 萬＋預熱 18 萬），<b>損益兩平約 650 台（募資額約 NT$98 萬）</b>。'
    '與代理模式相比：基準情境（900 台）淨利約 13 萬（10%），與代理模式相當——NRE 在此量級恰好抵銷單位成本優勢；'
    '但每台邊際貢獻高出約 37%，量級放大後差距快速拉開（樂觀 2,000 台：OEM 約 91 萬 vs 代理約 71 萬），'
    '且期末庫存與品牌資產可延伸至電商零售，非一次性生意。'
    '保守情境（400 台）虧損約 19 萬，較代理模式（約 10 萬）深，反映 NRE 與 MOQ 的前置承諾；'
    '止損機制（見第九章）因此更為重要。', st_body))

story.append(Paragraph('6.3 前期資金需求（不含量產貨款尾款）', st_h2))
story.append(tbl([
    ['項目', '金額（NT$）'],
    ['NRE（樣品、製版、包裝、BSMI、驗貨）', '141,000–290,000'],
    ...
    ['<b>合計</b>', '<b>約 551,000–1,090,000</b>'],
], [100 * mm, 70 * mm]))
story.append(Paragraph(
    '對比代理模式（約 35–75 萬），OEM 前期資金需求增加約 15–30 萬，主要來自 NRE 與訂金；'
    '尾款（約 NT$24 萬）由眾籌結案撥款支應。', st_body))
story.append(PageBreak())
...
story.append(tbl([
    ...
    ['募資未達損平（<650 台）', '中', '達標門檻設 NT$60 萬；未達損平僅出試產量、餘轉零售、停止追單'],
...
for x in [
    ...
    '財務結構：單位落地成本 NT$420（代理約 NT$600–650），損平約 650 台（募資額約 NT$98 萬）；基準情境淨利約 13 萬（10%）與代理相當，量級放大後優勢顯著（樂觀 2,000 台約 91 萬 vs 代理約 71 萬）；前期資金需求約 55–109 萬。',
