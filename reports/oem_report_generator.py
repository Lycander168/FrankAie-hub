...
for x in [
    ...
    '財務結構（NRE 另列立項投資）：單位落地成本 NT$420（代理約 NT$600–650），本檔損平僅約 270 台（募資額約 NT$40 萬），三情境皆正報酬——保守 +5.6 萬（10%）、基準 +38 萬（28%）、樂觀 +116 萬（38%）；基準情境檔期利潤即可全額回收 NRE。前期資金需求約 55–109 萬（含 NRE 與訂金）。',
