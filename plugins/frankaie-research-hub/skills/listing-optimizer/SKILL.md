...
5. **A+ 出版面**：A+ / EBC 不只給文字，直接把每個模組排成**可看的版面稿**
   （色彩字級取自 `design-system`），文案與版面一起交，不再拆成兩段流程。
6. **自檢評分**：用下方檢查表自評，弱項標註並給改寫建議。
...
## A+ / EBC 版面（文案 ＋ 版面一起交）
| 模組 | 文案內容 | 版面配置（直接排稿：構圖／層次／字級） | 需外部素材 |
|------|----------|--------------------------------------|-----------|
| 主圖橫幅 | 核心利益一句話 | | 產品去背圖 |
| 痛點對照 | before/after | | |
| 規格表 | 關鍵規格 | | |
...
> 版面直接出稿；「需外部素材」欄列出仍要實拍或去背的項目，先用佔位框排版，不等素材才動工。
> 定案前把版面稿（非文字描述）送 `validation-panel` 驗證。
...
- **平行**：A+ 版面直接出稿，變數取自 `design-system`；需要整套通路視覺分工時才升級到 `visual-team`；hook 句可呼叫 `ad-hook-writer`。
